...
#define CHECK_FRAME_ID( ident, b0, b1, b2, b3 ) ((( ident )[0]) == ( b0 ) && (( ident )[1]) == ( b1 ) && (( ident )[2]) == ( b2 ) && (( ident )[3]) == ( b3 ))
...
static qboolean Sound_ParseID3Tag( const byte *buffer, fs_offset_t filesize )
{
	...
	if(( header->major_ver != 3 && header->major_ver != 4 ) || header->minor_ver == 0xff )
	{
		Con_Reportf( S_ERROR "Sound_ParseID3Tag: invalid ID3v2 tag version %d.%d", header->major_ver, header->minor_ver );
		return false;
	}
	...
	{
		Con_Reportf( S_ERROR "Sound_ParseID3Tag: invalid tag length %u\n", tag_length );
		return false;
	}
	...
	if( FBitSet( header->flags, ID3V2_HEADER_EXTENDED_HEADER ))
	{
		...
		{
			Con_Reportf( S_ERROR "Sound_ParseID3Tag: invalid extended header length %u\n", ext_length );
			return false;
		}
	...
	while( buffer - buffer_begin < tag_length )
	{
		...
		{
			Con_Reportf( S_ERROR "Sound_ParseID3Tag: invalid frame length %u\n", frame_length );
			return false;
		}

		buffer += sizeof( *frame );

		// we're interested in TXXX tag
		if( CHECK_FRAME_ID( frame->frame_id, 'T', 'X', 'X', 'X' ))
		{
			string key, value;
			int32_t key_len, value_len;

			// can't support UTF-16 yet
			if( buffer[0] == 0x01 || buffer[0] == 0x02 )
			{
				Con_Reportf( S_ERROR "Sound_ParseID3Tag: unsupported TXXX tag encoding\n" );
				return false;
			}
			...
			{
				Con_Reportf( S_ERROR "Sound_ParseID3Tag: invalid TXXX description\n" );
				return false;
			}
