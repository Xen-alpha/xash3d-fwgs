...
CMenuBackgroundBitmap::bimage_t CMenuBackgroundBitmap::s_Titlelogo;
...
void CMenuBackgroundBitmap::Draw()
{
	...
	// Draw Title Logo
	EngFuncs::PIC_Set(s_Titlelogo.hImage, 255, 255, 255, 255);
	EngFuncs::PIC_DrawTrans((int) ScreenWidth * 3 / 4, s_Titlelogo.coord.y, (int)ceil(s_Titlelogo.size.w * xScale), (int)ceil(s_Titlelogo.size.h * yScale));
...
freefile:
	...
	// Now Load Title logo
	if (!EngFuncs::FileExists("resource/logo_game.tga", gamedirOnly))
		return loaded; // exit when no logo file exists
	s_Titlelogo.hImage = EngFuncs::PIC_Load("resource/logo_game.bmp", PIC_KEEP_SOURCE);
	s_Titlelogo.coord.x = 0; // will change this later
	s_Titlelogo.coord.y = 64;
	s_Titlelogo.size.w = EngFuncs::PIC_Width(s_Titlelogo.hImage);
	s_Titlelogo.size.h = EngFuncs::PIC_Height(s_Titlelogo.hImage);
